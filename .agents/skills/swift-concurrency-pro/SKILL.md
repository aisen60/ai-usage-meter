...
description: Writes and reviews Swift concurrency code for isolation, cancellation, structured tasks, process and stream lifetime, sendability, and async test correctness. Use when implementing or reviewing async/await, Task, actors, continuations, AsyncStream, Process, pipes, locks, or strict-concurrency migrations.
---
...
1. Inspect the project's compiler, Swift language mode, strict-concurrency settings, and default actor isolation before applying version-specific advice.
1. When Swift 6.2 or later features are in scope, check their behavior using `references/new-features.md`.
...
- Treat the checked-in Swift language mode and concurrency settings as authoritative. Do not enable strict concurrency, default actor isolation, or Swift 6 mode as an incidental fix.
...
- For process, pipe, and cancellation owners, verify that cleanup is idempotent and covers success, failure, timeout, and task cancellation.
- When implementing rather than reviewing, apply these rules directly and summarize meaningful safety decisions; do not emit a review report unless requested.
...
This project-local adaptation derives from Paul Hudson's Swift Concurrency Agent Skill and remains subject to the included MIT license.
