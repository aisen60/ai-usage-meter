...
Rarely correct. Check whether shedding actor isolation, task-local values, and priority is truly intentional. Prefer structured tasks; consider `@concurrent` only when the project uses Swift 6.2 or later and deliberately adopts that execution model. See `unstructured.md`.
