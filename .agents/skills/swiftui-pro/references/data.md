...
Before recommending Observation, inspect the deployment target and existing state architecture. `ObservableObject` remains a valid compatibility choice when Observation is unavailable or when migration is outside the requested scope.
...
- When the deployment target supports Observation and the project has chosen it, `@Observable` classes should be marked `@MainActor` unless the module uses Main Actor default isolation.
- In Observation-based code, use `@State` for ownership and `@Bindable` / `@Environment` for passing shared state.
- In an established `ObservableObject` codebase, preserve `@Published`, `@StateObject`, `@ObservedObject`, and `@EnvironmentObject` unless a deliberate migration provides concrete value and covers every binding boundary.
