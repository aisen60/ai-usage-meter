...
- Avoid platform color types in portable SwiftUI code when SwiftUI `Color` or asset catalog colors are sufficient. Use `NSColor` on macOS or `UIColor` on iOS when a platform semantic color or bridge is actually required.
