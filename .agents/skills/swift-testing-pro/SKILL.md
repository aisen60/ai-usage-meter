...
description: Writes, reviews, and improves tests that use the Swift Testing framework, and guides explicit migrations from XCTest. Use when a project already uses Swift Testing, when adding Swift Testing coverage by request, or when the user asks to migrate XCTest; do not trigger merely because a Swift project contains existing XCTest tests.
---
...
- Treat the project's installed toolchain, deployment target, language mode, and existing test framework as authoritative.
- Preserve existing XCTest suites unless the user requests migration. In a project standardized on XCTest, add XCTest coverage by default unless Swift Testing adoption is explicitly in scope.
- When Swift Testing is chosen, use only features available in the project's compiler and SDK; read `references/new-features.md` conditionally for version-specific APIs.
...
This project-local adaptation derives from Paul Hudson's Swift Testing Agent Skill and remains subject to the included MIT license.
